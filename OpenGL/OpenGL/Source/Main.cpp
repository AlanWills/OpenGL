...
int main()
{
  ...
  int width, height;
  unsigned char* image = SOIL_load_image("C:\\Users\\alawi\\Source\\Repos\\OpenGL\\OpenGL\\OpenGL\\Assets\\container.jpg", &width, &height, 0, SOIL_LOAD_RGB);
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  // Load, create texture and generate mipmaps
  image = SOIL_load_image("C:\\Users\\alawi\\Source\\Repos\\OpenGL\\OpenGL\\OpenGL\\Assets\\awesomeface.png", &width, &height, 0, SOIL_LOAD_RGB);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
  glGenerateMipmap(GL_TEXTURE_2D);
  SOIL_free_image_data(image);
